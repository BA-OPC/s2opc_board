...
SOPC_PubSubConfiguration* SOPC_PubSubConfig_GetStatic(void)
{
    ...
    if (alloc)
    {
        ...
        writerGroup = SOPC_PubSubConfig_SetPubMessageAt(connection, 0, 14, 1, 100.000000, -1, SOPC_SecurityMode_SignAndEncrypt, NULL, SOPC_MessageEncodeUADP, 0);
    ...
    if (alloc)
    {
        ...
        // WriterId = 0
        dataset = SOPC_PubSubConfig_InitDataSet(config, 0, writer, 0, 0, 1, 2);
        alloc = NULL != dataset;
    }
    if (alloc)
    {
        SOPC_PubSubConfig_SetPubVariableAt(dataset, 0, "ns=1;s=PubBool", SOPC_Boolean_Id, -1, NULL); // varBool
        SOPC_PubSubConfig_SetPubVariableAt(dataset, 1, "ns=1;s=PubString", SOPC_String_Id, -1, NULL); // varString
    }
    
    if (alloc)
    {
        alloc = SOPC_WriterGroup_Allocate_SecurityKeyServices_Array(writerGroup, 1);
    }
    ...
    if (alloc)
    {
        ...
        SOPC_ASSERT(NULL != writer);
        // WriterId = 0
        dataset = SOPC_PubSubConfig_InitDataSet(config, 1, writer, 0, 0, 1, 3);
        alloc = NULL != dataset;
    }
    if (alloc)
    {
        SOPC_PubSubConfig_SetPubVariableAt(dataset, 0, "ns=1;s=PubInt", SOPC_Int64_Id, -1, NULL); // varInt
        SOPC_PubSubConfig_SetPubVariableAt(dataset, 1, "ns=1;s=PubUInt", SOPC_UInt64_Id, -1, NULL); // varUInt
        uint32_t arrayDimensions_2[1] = {3};
        SOPC_PubSubConfig_SetPubVariableAt(dataset, 2, "ns=1;s=PubArrayInt", SOPC_Int64_Id, 1, arrayDimensions_2); // varInt
    }
